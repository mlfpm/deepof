    ...
    **kwargs,
    ...
    for key in tqdm.tqdm(to_preprocess.keys(), desc=f"{'Computing embeddings':<{PROGRESS_BAR_FIXED_WIDTH}}", unit="table"):
        ...
        if not contrastive:
            ...
            table_path = os.path.join(coordinates._project_path, coordinates._project_name, 'Tables', key + '_' + file_name + '_softc')
            soft_counts[key] = deepof.utils.save_dt(sc,table_path,coordinates._very_large_project)

        # save paths for modified tables
        table_path = os.path.join(coordinates._project_path, coordinates._project_name, 'Tables', key + '_' + file_name + '_embed')
