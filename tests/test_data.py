...
################
# Intermediarily added tests to make sure old and new preprocessing are identical
################
...
@contextmanager
def _patch_preprocess_to_new():
    """Temporarily make get_graph_dataset use preprocess_new."""
    orig = deepof.data.TableDict.preprocess
    deepof.data.TableDict.preprocess = deepof.data.TableDict.preprocess_old
    try:
        yield
    finally:
        deepof.data.TableDict.preprocess = orig


def _scaler_state(sc):
    """Extract comparable learned attributes from sklearn scalers."""
    if sc is None:
        return None
    state = {"__class__": sc.__class__.__name__}
    for attr in (
        "mean_",
        "scale_",
        "var_",
...
def _assert_nested_equal(a, b):
    """Recursive equality for nested structures, tolerant to numpy/torch-ish payloads."""
    # numpy arrays
    ...
    if hasattr(a, "detach") and hasattr(b, "detach"):
        _assert_nested_equal(a.detach().cpu().numpy(), b.detach().cpu().numpy())
        return
    ...
    if isinstance(a, dict) and isinstance(b, dict):
        ...
        for k in a.keys():
            _assert_nested_equal(a[k], b[k])
        return

    # sequences
    if isinstance(a, (list, tuple)) and isinstance(b, (list, tuple)) or (isinstance(a,pd.Index) and isinstance(b,pd.Index)):
        assert len(a) == len(b)
        for x, y in zip(a, b):
...
def _assert_global_scaler_equal(gs_old, gs_new):
    # Both None
    if gs_old is None and gs_new is None:
        return

    # For this test we expect legacy dict format (or None)
    assert isinstance(gs_old, dict) and isinstance(gs_new, dict)
    assert set(gs_old.keys()) == set(gs_new.keys())

    # Compare non-scaler metadata directly
    for k in ("kind", "speed_mode", "dist_mode", "coord_mode", "log_distances"):
        assert gs_old.get(k) == gs_new.get(k)

    # Compare fitted scaler objects by learned state
    for k in ("speed", "dist", "dist_inner", "dist_intra", "coord"):
        st_old = _scaler_state(gs_old.get(k))
        st_new = _scaler_state(gs_new.get(k))
        _assert_nested_equal(st_old, st_new)


@settings(deadline=None, max_examples=15)
@given(
    mode=st.sampled_from(["single", "multi", "madlc"]),
    sampler=st.data(),
    random_id=st.text(alphabet=string.ascii_letters, min_size=50, max_size=50),
    test_videos=st.one_of(st.just(1), st.just(["test"])),
    full_nan_table=st.booleans(),
...
def test_get_graph_dataset_preprocess_new_matches_old(
    mode,
    sampler,
    random_id,
    test_videos,
    full_nan_table,
    dist_standardize_groups,
    speed_standardize_groups,
):
    if mode == "multi":
        animal_ids = ["B", "W"]
    elif mode == "madlc":
    ...
    dist_standardize = "groupwise" if dist_standardize_groups else "per_column"
    speed_standardize = "groupwise" if speed_standardize_groups else "per_column"

    # Draw once and reuse across old/new runs
    animal_id_arg = sampler.draw(st.one_of(st.just(None), st.just(animal_ids[0])))
    scale_arg = sampler.draw(
        st.one_of(st.just("standard"), st.just("minmax"), st.just("robust"), st.just(False))
    )

    base_project_path = os.path.join(".", "tests", "test_examples", f"test_{mode}_topview")
    video_path = os.path.join(base_project_path, "Videos")
    table_path = os.path.join(base_project_path, "Tables")

    prun_old = deepof.data.Project(
        project_path=base_project_path,
        video_path=video_path,
        table_path=table_path,
        project_name=f"deepof_project_{random_id}_old",
        arena="circular-autodetect",
        video_scale="380 mm",
        video_format=".mp4",
        animal_ids=animal_ids,
    ...
    prun_old._frame_rate=25

    prun_new = deepof.data.Project(
        project_path=base_project_path,
        video_path=video_path,
        table_path=table_path,
        project_name=f"deepof_project_{random_id}_new",
        arena="circular-autodetect",
        video_scale="380 mm",
        video_format=".mp4",
        animal_ids=animal_ids,
    ...
    prun_new._frame_rate=25

    try:
        if full_nan_table:
            # simulate missing data (apply identically to both projects)
            for prun in (prun_old, prun_new):
                key_with_nans = list(prun._tables.keys())[0]
                prun._tables[key_with_nans].iloc[::] = np.nan
                prun._distances[key_with_nans].iloc[::] = np.nan
                prun._angles[key_with_nans].iloc[::] = np.nan
        ...
        # Old behavior
        out_old = prun_old.get_graph_dataset(
            animal_id=animal_id_arg,
            scale=scale_arg,
            test_videos=test_videos,
            dist_standardize=dist_standardize,
            speed_standardize=speed_standardize,
        ...
        # New behavior (patch preprocess->preprocess_new)
        with _patch_preprocess_to_new():
            out_new = prun_new.get_graph_dataset(
                animal_id=animal_id_arg,
                scale=scale_arg,
                test_videos=test_videos,
                dist_standardize=dist_standardize,
        ...
        graph_dset_old, meta_info_old, adj_old, _to_preprocess_old, gs_old = out_old
        graph_dset_new, meta_info_new, adj_new, _to_preprocess_new, gs_new = out_new

        # Hard equality checks where feasible
        np.testing.assert_allclose(adj_old, adj_new, rtol=0, atol=0, equal_nan=True)
        _assert_nested_equal(meta_info_old, meta_info_new)
        _assert_global_scaler_equal(gs_old, gs_new)

        # Graph dataset can be nested; compare recursively
        _assert_nested_equal(graph_dset_old, graph_dset_new)

        # Keep the existing invariants from your original test
        assert isinstance(graph_dset_old, tuple)
        assert isinstance(adj_old, np.ndarray)
        assert isinstance(meta_info_old, dict)

        if full_nan_table:
            assert len(graph_dset_old[0]) == 0
            assert isinstance(meta_info_old, dict) and len(meta_info_old["shape_train"]) == 3

    finally:
        rmtree(os.path.join(base_project_path, f"deepof_project_{random_id}_old"))
        rmtree(os.path.join(base_project_path, f"deepof_project_{random_id}_new"))