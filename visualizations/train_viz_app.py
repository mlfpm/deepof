...
def flip_axes(projections):
    """Flips projected PCA axes for subsequent latent representations of the input to remain closer to one another in the visualization"""
...
fig.update_xaxes(showgrid=False, range=[-0.2, 1.2])
fig.update_yaxes(showgrid=False, range=[-0.2, 1.2])
...
fig2.update_yaxes(showgrid=False, range=[0, pttest.shape[0] / 1.5])
...
dfencs["trajectories"] = np.tile(pttest[:, 6, 1], 4)
...
fig3.update_xaxes(showgrid=False, range=[-2, 2])
fig3.update_yaxes(showgrid=False, range=[-2, 2])
...
app = dash.Dash()
app.css.append_css({"external_url": "dash_style.css"})

app.layout = html.Div(
    [   html.H1(children='DLC Analysis GMVAE training demo', className="row")
        html.Div(
            [
